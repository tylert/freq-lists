...
def output_chirp_channels(channels, max_name_length=8):
    ...
    for channel in channels:
        ...
        if 'CtcssDecode' in channel.keys():
            if channel['CtcssDecode'] is not None \
        ...
        # CTCSS tone type
        if 'CtcssDecode' in channel.keys() and 'CtcssEncode' in channel.keys():
            if channel['CtcssDecode'] is not None and channel['CtcssEncode'] is not None \
        ...
        print(f"{location},{name},{frequency:.6f},{duplex},{offset:.6f},{tone},{r_tone_freq},{c_tone_freq},{dtcs_code},{dtcs_polarity},{mode},{tstep:.2f},,,,,,")
        location += 1


def output_rt_systems_1_channels():
    print('Receive Frequency,Transmit Frequency,Offset Frequency,Offset Direction,Repeater Use,Operating Mode,Name,Sub Name,Tone Mode,CTCSS,Rx CTCSS,DCS,DCS Polarity,Skip,Step,Digital Squelch,Digital Code,Your Callsign,Rpt-1 CallSign,Rpt-2 CallSign,LatLng,Latitude,Longitude,UTC Offset,Bank,Bank Channel Number,Comment')
...
def output_rt_systems_2_channels():
    print('Channel Number,Receive Frequency,Transmit Frequency,Offset Frequency,Offset Direction,Operating Mode,Name,Tone Mode,CTCSS,Rx CTCSS,DCS,DCS Polarity,Skip,Step,Bank,Bank Channel Number,Comment,Digital Squelch,Digital Code,Your Callsign,Rpt-1 CallSign,Rpt-2 CallSign')
...
@click.option('--max_name_length', '-m', default=8, help='Maximum length of channel names (default 8).')
